 ...
 * @version: V1.0
 * @Author: snow.wei
 * @Date: 2022-02-24 17:15:50
 * @LastEditors: snow.wei
 * @LastEditTime: 2022-02-24 19:26:01
 */
package config
...
func init() {
	config.Add("log", func() map[string]interface{} {
		return map[string]interface{}{
			...
			// 日志级别，必须是以下这些选项：
			// "debug" -- 信息量大，一般调试时打开。系统模块详细运行的日志，例如 HTTP请求、数据库请求、发送邮件、发送短信
			// ”info“ -- 业务级别的运行日志，如用户登录、用户退出、订单撤销
			// "warn" -- 感兴趣、需要引起关注的信息。例如，调试时候打印调试信息（命令行输出回有高亮）
			// "error" -- 记录错误信息。Panic 或者Error。如数据库连接错误、HTTP 端口被占用等。一般生产环境使用的等级。

			// 以上级别从低到高，level值设置的级别越高，记录到日志的信息就越少
			...
			"max_backup": config.Env("LOG_MAX_BACK", 5),
