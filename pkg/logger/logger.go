...
func InitLogger(filename string, maxSize, maxBackup, maxAge int, compress bool,
	logType string, level string) {

	// 获取日志写入介质
	writeSyncer := getLogWriter(filename, maxSize, maxBackup, maxAge, compress, logType)

	// 设置日志等级， 具体请见 config/log.go文件
	logLevel := new(zapcore.Level)

	if err := logLevel.UnmarshalText([]byte(level)); err != nil {
		fmt.Println("日志初始化错误，日志级别设置有误。请修改 config/log.go 文件中 log.level 配置项")
	}
	...
	// 将自定义的logger 替换为全局的 logger
	// zap.L().Fatal() 调用时，就会使用我们自定的Logger
...
func getEncoder() zapcore.Encoder {
	...
	encoderConfig := zapcore.EncoderConfig{
		...
		LevelKey:       "level",
		NameKey:        "logger",
		CallerKey:      "caller", // 代码调用，如paginator/paginator.go
		FunctionKey:    zapcore.OmitKey,
		MessageKey:     "messages",
		StacktraceKey:  "stacktrace",
		LineEnding:     zapcore.DefaultLineEnding,        // 每行日志的结尾添加 “\n”
		EncodeLevel:    zapcore.CapitalColorLevelEncoder, // 日志级别名称大写，如ERROR、INFO
		EncodeTime:     customTimeEncoder,                // 时间格式，我们自定义为 2006-01-02 15:04:05
		EncodeDuration: zapcore.SecondsDurationEncoder,   // 执行时间，以秒为单位
		EncodeCaller:   zapcore.ShortCallerEncoder,       // Caller 短格式，如：type/converter.go：17 长格式为绝对路径
	}
...
// Dump 调试专用，不会中断程序，会在终端打印出Waring 消息
// 第一个参数会使用json.Marshal 进行渲染，第二个参数消息（可选）
// 			logger.Dump(user.User{Name:"user"})
...
// LogWarnIf 当 err != nil 时记录 waring等级的日志
func LogWarnIf(err error) {
	if err != nil {
		Logger.Warn("Error Occured", zap.Error(err))
	}
}

// LogInfoIf 当 err != nil 时记录 info等级的日志
...
// DebugJSON 记录一条字符串类型的Debug 的日志，调用示例：
//      logger.DebugString("SMS", "短信内容", string(result.RawResponse))
