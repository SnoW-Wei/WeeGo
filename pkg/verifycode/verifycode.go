...
func (vc *VerifyCode) CheckAnser(key string, answer string) bool {
	...
	if !app.IsProduction() && (strings.HasPrefix(key, config.GetString("verifycode.debug_email_suffix")) || strings.HasPrefix(key, config.GetString("verifycode.debug_phone_suffix"))) {
		return true
	}
