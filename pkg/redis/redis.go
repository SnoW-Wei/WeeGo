...
// Increment 当参数只有 1时，为key ,为值增加 1.
// 当参数有 2个时，第一个参数为 key，第二个参数为要增加的值 int64 类型。
...
// Decrement 当参数只有1 个时，为key ,其值减去 1。
// 当参数有 2个时，第一个参数 为 key，第二个参数为要减去的值 int64 类型。
